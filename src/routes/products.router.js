...
router.get("/:idProd", async (req, res) => {
    try {
      ...
      if (!product) res.status(404).json({ msg: "product no encontrado" });
...
router.put("/:idProd", async (req, res) => {
    try {
      ...
      if (!prodUpd) res.status(404).json({ msg: "Error actualizando prod" });
...
router.delete("/:idProd", async (req, res) => {
    try {
      ...
      if(!delProd) res.status(404).json({ msg: "Error eliminar producto" });
      else res.status(200).json({msg : `producto id: ${idProd} eliminado correctamente`})
    } catch (error) {
      res.status(500).json({ msg: error.message });
    }
  });
